...
typedef struct VariablesGradAllocator {
  Variable
      *independent_var; // independent var whose gradient we are calculating
...
void init_grad_buf(VariablesGradAllocator *var_grad_allocator) {
  VariablesGradAllocator temp = {0};
  *var_grad_allocator = temp;
}

// Initialize gradient buffer
void init_grad_bufs(VariablesGradAllocator **var_grad_allocators, size_t size) {
  for (size_t alloc_idx = 0; alloc_idx < size; alloc_idx++) {
    var_grad_allocators[alloc_idx] =
        (VariablesGradAllocator *)malloc(sizeof(VariablesGradAllocator));
    init_grad_buf(var_grad_allocators[alloc_idx]);
  }
}
...
Variable *relu(Variable *x) {
  ...
  // Create a new Variable with the computed value and connections
  Variable *newValue = malloc(sizeof *newValue);
  init_var(newValue, (x->val > 0) ? x->val : 0);
  newValue->children = children;
  newValue->local_grads = local_grads;
  newValue->n_children = 1;
...
// Variable
void compute_grads(VariablesGradAllocator *var_grad_store,
                   Variable *independent_var, int path_value) {
  // TODO: Unnecessary?
  if (independent_var == NULL || var_grad_store == NULL) {
    return;
  }

  for (int child_index = 0; child_index < independent_var->n_children;
       child_index++) {
    Variable *var = independent_var->children[child_index];
    int path_to_child_val = var->local_grads != NULL
                                ? path_value * var->local_grads[child_index]
                                : path_value;
    ...
    if (var_grad_store->size <= child_index) {
      var_grad_store->dependent_vars =
          (Variable **)realloc(var_grad_store->dependent_vars, child_index + 1);
      var_grad_store->gradients =
          (Variable **)realloc(var_grad_store->gradients, child_index + 1);
    }

    // TODO: I'm using "+=" here, are the values for the ints here 0 after
    // malloc? Because if they aren't, this will break 💀
    ...
    if (var_grad_store->gradients[child_index] == NULL) {
      ...
      Variable **new_vars_arr = (Variable **)malloc(sizeof *new_vars_arr);
      init_var(new_var, 0);
...
void get_gradients(VariablesGradAllocator **grad_buf,
  ...
  for (int root_index = 0; root_index < n_vars; root_index++) {
    ...
    grad_buf =
        realloc(grad_buf, (root_index + 1) * sizeof(VariablesGradAllocator *));
